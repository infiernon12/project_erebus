...
from alex_vibe.alex_brain import process_user_return
...
def run_test():
    ...
    db.update_alex_emotions_and_fatigue(
        user_id,
        dopamine_delta=0.5 - db.get_alex_emotions(user_id).get("dopamine", 0.5),
        serotonin_delta=0.5 - db.get_alex_emotions(user_id).get("serotonin", 0.5),
        noradrenaline_delta=0.4 - db.get_alex_emotions(user_id).get("noradrenaline", 0.4),
        acetylcholine_delta=0.6 - db.get_alex_emotions(user_id).get("acetylcholine", 0.6),
        gaba_delta=0.5 - db.get_alex_emotions(user_id).get("gaba", 0.5),
        oxytocin_delta=0.4 - db.get_alex_emotions(user_id).get("oxytocin", 0.4),
        glutamate_delta=0.5 - db.get_alex_emotions(user_id).get("glutamate", 0.5),
        endorphins_delta=0.3 - db.get_alex_emotions(user_id).get("endorphins", 0.3),
        fatigue_delta=-db.get_alex_emotions(user_id).get("fatigue", 0.0),
    ...
    for i in range(1, 5):
        ...
        db.update_alex_leave_status(user_id, now_str, "test")
        ...
        hyps = db.get_alex_hypotheses(user_id, status=None)
        hyp_str = "None"
        for h in hyps:
    ...
    db.update_alex_leave_status(user_id, expected_late.strftime("%Y-%m-%d %H:%M:%S"), "test")
